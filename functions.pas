...
uses
  ...
  type
   { TFile }
    ...
    TArduinoFile = Class
          constructor create(fname:string;data:string); overload;
...
constructor TArduinoFile.create(fname: string);
begin
    Filecontent:=TStringList.Create;
     datasource:=TDataSource.Create(nil);
     dataset:=TSdfDataSet.Create(nil);
...
function TArduinoFile.addData(adata: string):boolean;
var
  aStringStream:TStringStream;
begin
    if aData='EOF' then
    begin
      try
     aStringStream:=TStringStream.Create(TrimRight(ruledata));
     dataset.LoadFromStream(aStringStream);
     finally
     aStringStream.Free;
     end;
